...
class MIXREG(ActorCriticRLModel):
    """
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
                 ...
                 full_tensorboard_log=False, seed=None, n_cpu_tf_sess=None, mix_alpha=0.2):
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            assert issubclass(self.policy, ActorCriticPolicy), "Error: the input policy for the MIXREG model must be " \
            ...
            with self.graph.as_default():
                ...
                act_model = self.policy(self.sess, self.observation_space, self.action_space, self.n_envs, 1,
                                        n_batch_step, reuse=False, mix_alpha=self.mix_alpha, **self.policy_kwargs)
                with tf.variable_scope("train_model", reuse=True,
                                       custom_getter=tf_util.outer_scope_getter("train_model")):
                    train_model = self.policy(self.sess, self.observation_space, self.action_space,
                                              self.n_envs // self.nminibatches, self.n_steps, n_batch_train,
                                              reuse=True, mix_alpha=self.mix_alpha, **self.policy_kwargs)
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.old_neglog_pac_ph = coeff * tf.gather(self.old_neglog_pac_ph, indices, axis=0) + (1 - coeff) * tf.gather(self.old_neglog_pac_ph, other_indices, axis=0)
                    self.old_vpred_ph = coeff * tf.gather(self.old_vpred_ph, indices, axis=0) + (1 - coeff) * tf.gather(self.old_vpred_ph, other_indices, axis=0)
                    self.rewards_ph = coeff * tf.gather(self.rewards_ph, indices, axis=0) + (1 - coeff) * tf.gather(self.rewards_ph, other_indices, axis=0)
                    self.advs_ph = coeff * tf.gather(self.advs_ph, indices, axis=0) + (1 - coeff) * tf.gather(self.advs_ph, other_indices, axis=0)
                    self.action_ph = tf.gather(self.action_ph, indices, axis=0)
                    ###---------------------------###
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="MIXREG",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                rollout = self.runner.run(callback)#TODO: FAILS RIGHT HERE
                # Unpack
